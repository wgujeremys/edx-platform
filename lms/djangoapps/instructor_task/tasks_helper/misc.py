...
def generate_anonymous_ids(_xmodule_instance_args, _entry_id, course_id, task_input, action_name):  # lint-amnesty, pylint: disable=too-many-statements
    ...
    TASK_LOG.info('Testing log: Starting task execution for generation of Anonymize IDs report.')
    ...
    current_step = {'step': "Started Computing rows"}
    _log_and_update_progress(current_step)
    ...
    current_step = {'step': "Completed Computing rows"}
    _log_and_update_progress(current_step)
